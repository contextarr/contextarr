...
- Local Fastify API for packs, records, search, health, exposure readiness, exports, composition, collectors, Draft Intake candidates, activation plans, and dry-run activation proof, backup, and restore surfaces.
...
- Draft Intake shows metadata-only candidate inventories, read-only activation plans, and dry-run activation proof; it does not activate, index, export, or expose candidates through MCP.
...
- Draft Intake v0 is read-only; activation plans and dry-run proof prepare manual next steps, but it does not include approve, promote, or activate actions.
