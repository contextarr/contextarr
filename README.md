...
- Local Fastify API for packs, records, search, health, exposure readiness, exports, composition, collectors, Draft Intake candidates and activation plans, backup, and restore surfaces.
...
- Draft Intake shows metadata-only candidate inventories and read-only activation plans; it does not activate, index, export, or expose candidates through MCP.
...
- Draft Intake v0 is read-only; activation plans prepare manual next steps, but it does not include approve, promote, or activate actions.
