...
- `contextarr inspect <pack-id> --kind pack --readiness`
...
Validation is read-only for Context Packs, Skills, and Agent Kits. `review-candidates` is read-only and reports sanitized metadata for untrusted draft/composed/quarantine Context Pack folders without indexing, activating, exporting, or returning record bodies. `inspect --readiness` is read-only and reports Context Pack export/MCP eligibility reasons without changing export behavior. Render and export commands write generated artifacts to the requested output folder and never mutate source files.
