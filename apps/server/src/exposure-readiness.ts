...
type ExposureIssueSeverity = "blocker" | "warning";
...
export function getPackExposureReadiness(
  ...
  const profiles = toProfileReadiness(pack, packBlocked ? packBlockers : []);
  ...
  const blockedProfiles = profiles.filter((profile) => profile.blockers.length > 0).length;
  const warningProfiles = profiles.filter((profile) => profile.warnings.length > 0).length;
  ...
  return {
    ...
    summary: {
      ...
      blockedRecords,
      warningRecords,
      exportProfileCount: profiles.length,
      exportEligibleProfiles: profiles.filter((profile) => profile.exportEligible).length,
      blockedProfiles,
      warningProfiles
    },
...
function validatePackSafely(packPath: string): ValidationResult {
  try {
    ...
    throw new ExposureReadinessError("pack.validation_failed", error instanceof Error ? error.message : String(error), 400);
  }
}
...
function scanPackSafely(packPath: string, packId: string): SecurityScannerReportV1 {
  try {
    ...
    throw new ExposureReadinessError("pack.scanning_failed", error instanceof Error ? error.message : String(error), 400);
  }
}
...
function toRecordReadiness(record: IndexedRecord, packBlockers: ExposureIssue[]): ExposureRecordReadiness {
  ...
  if (record.privacy === "secret") {
    ...
    blockers.push(issue("record.privacy.not_public_safe", "blocker", `Default export and MCP exposure require public_safe privacy; got ${record.privacy}.`));
  }
  ...
  if (!["source_backed", "verified"].includes(record.sourceStatus)) {
