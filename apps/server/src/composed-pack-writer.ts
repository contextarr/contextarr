...
function writeComposedPack(
  ...
  for (const [index, record] of options.records.entries()) {
    ...
    sources.push(buildComposedSource(sourceId, record, index));
...
function buildComposedSource(sourceId: string, record: ComposeDraftRecord, index: number): Source {
  const firstSource = record.resolvedSources[0];
  return {
    id: sourceId,
    type: "contextarr_record",
    title: `${record.title} source record`,
    path: `contextarr://packs/${record.packId}/records/${record.id}`,
