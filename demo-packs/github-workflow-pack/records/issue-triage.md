---
id: github-workflow-pack.issue-triage
title: Issue Triage
...
sources:
  - github-workflow-pack-issue-triage-source
...
Issue Triage describes a fictional open-source repository called Northstar Docs that uses reviewed issues, pull requests, and release notes. It gives maintainers who want AI agents to understand contribution workflow without live GitHub access a reviewed, local, public-safe context record that can be rendered for humans and reused in target-specific AI briefs.

## Key Facts

| Area | Demo detail | Why it matters |
| --- | --- | --- |
| Scope | Labels separate kind, area, priority, and review state. | Keeps the pack specific without exposing private operational data. |
| Review use | Bugs need reproduction context and expected behavior before assignment. | Keeps the pack specific without exposing private operational data. |
| AI value | Ideas stay parked until they map to roadmap stage or launch friction. | Keeps the pack specific without exposing private operational data. |
...
- Record ID: `github-workflow-pack.issue-triage`
