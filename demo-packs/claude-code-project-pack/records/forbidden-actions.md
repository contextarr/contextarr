---
id: claude-code-project-pack.forbidden-actions
...
sources:
  - claude-code-project-pack-forbidden-actions-source
...
Forbidden Actions describes a fictional TypeScript repository named Meridian Notes that uses AI coding agents for scoped implementation work. It gives maintainers preparing Claude Code, Codex, and local coding-agent handoffs a reviewed, local, public-safe context record that can be rendered for humans and reused in target-specific AI briefs.

## Key Facts

| Area | Demo detail | Why it matters |
| --- | --- | --- |
| Scope | Never run scripts from imported packs or Skills. | Keeps the pack specific without exposing private operational data. |
| Review use | Never add telemetry, hosted vault behavior, unapproved remote access, or mutating MCP tools. | Keeps the pack specific without exposing private operational data. |
| AI value | Never overwrite unrelated user edits to make a test easier. | Keeps the pack specific without exposing private operational data. |
...
- Record ID: `claude-code-project-pack.forbidden-actions`
