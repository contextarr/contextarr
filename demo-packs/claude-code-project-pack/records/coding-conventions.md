---
id: claude-code-project-pack.coding-conventions
...
sources:
  - claude-code-project-pack-coding-conventions-source
...
Coding Conventions describes a fictional TypeScript repository named Meridian Notes that uses AI coding agents for scoped implementation work. It gives maintainers preparing Claude Code, Codex, and local coding-agent handoffs a reviewed, local, public-safe context record that can be rendered for humans and reused in target-specific AI briefs.

## Key Facts

| Area | Demo detail | Why it matters |
| --- | --- | --- |
| Scope | Use small TypeScript modules and deterministic JSON outputs for agent-facing surfaces. | Keeps the pack specific without exposing private operational data. |
| Review use | Prefer schema validation and structured parsing over ad hoc string checks. | Keeps the pack specific without exposing private operational data. |
| AI value | Keep UI copy terse; put long explanations in docs or release notes. | Keeps the pack specific without exposing private operational data. |
...
- Record ID: `claude-code-project-pack.coding-conventions`
