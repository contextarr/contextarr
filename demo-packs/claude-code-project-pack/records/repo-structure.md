---
id: claude-code-project-pack.repo-structure
...
sources:
  - claude-code-project-pack-repo-structure-source
...
Repo Structure describes a fictional TypeScript repository named Meridian Notes that uses AI coding agents for scoped implementation work. It gives maintainers preparing Claude Code, Codex, and local coding-agent handoffs a reviewed, local, public-safe context record that can be rendered for humans and reused in target-specific AI briefs.

## Key Facts

| Area | Demo detail | Why it matters |
| --- | --- | --- |
| Scope | App folders: dashboard UI, local API service, CLI surface, and package validators. | Keeps the pack specific without exposing private operational data. |
| Review use | Shared packages: export profiles, review candidates, renderer, and security scanner. | Keeps the pack specific without exposing private operational data. |
| AI value | Review expectation: quote file areas by feature responsibility, not by private local paths. | Keeps the pack specific without exposing private operational data. |
...
- Record ID: `claude-code-project-pack.repo-structure`
