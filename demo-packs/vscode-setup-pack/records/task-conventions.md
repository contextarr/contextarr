---
id: vscode-setup-pack.task-conventions
...
sources:
  - vscode-setup-pack-task-conventions-source
...
Task Conventions describes a fictional workstation profile for repeatable editor setup across TypeScript, Markdown, and local API projects. It gives developers who want AI help with editor context without pasting private settings files a reviewed, local, public-safe context record that can be rendered for humans and reused in target-specific AI briefs.

## Key Facts

| Area | Demo detail | Why it matters |
| --- | --- | --- |
| Scope | Tasks are described by purpose: validate, test, build, preview, and docs check. | Keeps the pack specific without exposing private operational data. |
| Review use | Dangerous tasks are documented as manual-owner-only. | Keeps the pack specific without exposing private operational data. |
| AI value | Agents may suggest which gate applies but should not execute editor tasks from the pack. | Keeps the pack specific without exposing private operational data. |
...
- Record ID: `vscode-setup-pack.task-conventions`
