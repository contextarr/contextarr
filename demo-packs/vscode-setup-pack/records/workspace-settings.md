---
id: vscode-setup-pack.workspace-settings
...
sources:
  - vscode-setup-pack-workspace-settings-source
...
Workspace Settings describes a fictional workstation profile for repeatable editor setup across TypeScript, Markdown, and local API projects. It gives developers who want AI help with editor context without pasting private settings files a reviewed, local, public-safe context record that can be rendered for humans and reused in target-specific AI briefs.

## Key Facts

| Area | Demo detail | Why it matters |
| --- | --- | --- |
| Scope | Settings are grouped by formatting, diagnostics, explorer behavior, and terminal safety. | Keeps the pack specific without exposing private operational data. |
| Review use | Private paths are represented as role labels such as workspace-root and local-cache. | Keeps the pack specific without exposing private operational data. |
| AI value | Unknown settings should be reviewed before being copied into a new project. | Keeps the pack specific without exposing private operational data. |
...
- Record ID: `vscode-setup-pack.workspace-settings`
