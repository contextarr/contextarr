---
id: obsidian-vault-pack.import-boundaries
...
sources:
  - obsidian-vault-pack-import-boundaries-source
...
Import Boundaries describes a fictional Markdown vault called Atlas Notes with local knowledge, daily notes, and export rules. It gives Obsidian and Markdown users turning notes into reviewed AI context a reviewed, local, public-safe context record that can be rendered for humans and reused in target-specific AI briefs.

## Key Facts

| Area | Demo detail | Why it matters |
| --- | --- | --- |
| Scope | Import creates draft candidates, not trusted records. | Keeps the pack specific without exposing private operational data. |
| Review use | Importer summaries preserve local file ownership and review state. | Keeps the pack specific without exposing private operational data. |
| AI value | Unknown attachments are skipped or quarantined for review. | Keeps the pack specific without exposing private operational data. |
...
- Record ID: `obsidian-vault-pack.import-boundaries`
