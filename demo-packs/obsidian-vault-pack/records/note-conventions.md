---
id: obsidian-vault-pack.note-conventions
...
sources:
  - obsidian-vault-pack-note-conventions-source
...
Note Conventions describes a fictional Markdown vault called Atlas Notes with local knowledge, daily notes, and export rules. It gives Obsidian and Markdown users turning notes into reviewed AI context a reviewed, local, public-safe context record that can be rendered for humans and reused in target-specific AI briefs.

## Key Facts

| Area | Demo detail | Why it matters |
| --- | --- | --- |
| Scope | Reviewed notes include summary, status, source, owner role, and last-reviewed date. | Keeps the pack specific without exposing private operational data. |
| Review use | Draft notes can be imported but stay out of exports and MCP. | Keeps the pack specific without exposing private operational data. |
| AI value | Ambiguous notes become review queue items. | Keeps the pack specific without exposing private operational data. |
...
- Record ID: `obsidian-vault-pack.note-conventions`
