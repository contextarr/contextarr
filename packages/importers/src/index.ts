...
function writeDraftPack(packPath: string, preview: DraftPackPreview, generatedAt: string): void {
  ...
  const recordsBySourceId = new Map(preview.records.map((record) => [record.sourceId, record]));
  const sources = preview.sources.map((source): Source => {
    const rawPath = `raw/${safeRawSourceFileName(source.id)}.md`;
    fs.writeFileSync(
      path.join(packPath, rawPath),
      buildImportedRawSourceNote(preview.kind, source, recordsBySourceId.get(source.id)),
      "utf8"
    );
...
function writeDraftSkill(skillPath: string, preview: DraftSkillPreview, generatedAt: string): void {
  ...
  fs.mkdirSync(path.join(skillPath, "raw"), { recursive: true });
  ...
  const documentsBySourceId = new Map(preview.documents.map((document) => [document.sourceId, document]));
  const sources = preview.sources.map((source): Source => {
    const rawPath = `raw/${safeRawSourceFileName(source.id)}.md`;
    fs.writeFileSync(
      path.join(skillPath, rawPath),
      buildImportedSkillRawSourceNote(preview.kind, source, documentsBySourceId.get(source.id)),
      "utf8"
    );
...
function buildImportedRawSourceNote(kind: ResolvedImporterKind, source: ImportedSource, record: ImportedRecord | undefined): string {
  return [
    `# ${source.title} Source Note`,
    "",
    "Pack-local source note generated by the Contextarr importer.",
    "",
    "## Import Metadata",
    "",
    `- Import kind: ${kind}`,
    ...
    `- Original reference: ${formatRawReference(source.path)}`,
    "- Review state: unreviewed draft",
    "",
    "## Imported Content Snapshot",
    "",
    record?.body.trim() || "No imported content snapshot was available for this source."
  ].join("\n");
}

function buildImportedSkillRawSourceNote(
  kind: ResolvedSkillImporterKind,
  source: ImportedSource,
  document: ImportedSkillDocument | undefined
): string {
  return [
    `# ${source.title} Source Note`,
    "",
    "Pack-local source note generated by the Contextarr Skill importer.",
    "",
    "## Import Metadata",
    "",
    `- Import kind: ${kind}`,
    ...
    "## Imported Content Snapshot",
    "",
    document?.body.trim() || "No imported instruction snapshot was available for this source."
  ].join("\n");
}

function formatRawReference(value: string): string {
  return `\`${value.replace(/`/g, "'")}\``;
}

function safeRawSourceFileName(sourceId: string): string {
  return safeFileName(sourceId.replace(/\./g, "-")).replace(/^-+|-+$/g, "") || "source";
}
