...
function buildStarterPreview(
  ...
  const sources = records.map(
    (record): Source => ({
      ...
      path: `raw/${safeFileName(record.slug)}-source.md`,
      license: "N/A",
...
function writeStarterDraftPack(
  ...
  for (const source of preview.sources) {
    const record = recordsBySourceId.get(source.id);
    if (source.path) {
      fs.writeFileSync(
        path.join(packPath, source.path),
        buildCollectorRawSourceNote(definition, source, record),
        "utf8"
      );
    }
  }
...
function buildCollectorRawSourceNote(
  definition: ContextPackCollectorDefinition,
  source: Source,
  record: StarterPackPreview["records"][number] | undefined
): string {
  return [
    `# ${source.title} Source Note`,
    "",
    "Pack-local source note generated by a Contextarr collector.",
    "",
    "## Collector Metadata",
    "",
    `- Collector: ${definition.name}`,
    `- Collector ID: ${definition.id}`,
    `- Source ID: ${source.id}`,
    "- Review state: unreviewed draft",
    "",
    "## Starter Content Snapshot",
    "",
    record?.body.trim() || "No starter record body was available for this source."
  ].join("\n");
}
